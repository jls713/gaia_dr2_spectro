...
//=============================================================================
double new_prior_2018_broad_age::bulge_prior(VecDoub X, double Z, double tau){
    double xrot = X[0]*cos(bulge_phi)-X[1]*sin(bulge_phi);
    double yrot = -X[0]*sin(bulge_phi)-X[1]*cos(bulge_phi);
    return bulge_norm*
           GFunction(Z-bulge_metal_mean,bulge_metal_disp)*
...
double new_prior_2018_broad_age::thin_prior(double R, double z, double Z, double tau){
    double thin_age = 0.;
    if(tau<thin_age_mean)
        thin_age = exp(0.119*tau);
    else
...
double new_prior_2018_broad_age::thick_prior(double R, double z, double Z, double tau){
    return thick_norm*
           GFunction(Z-thick_metal_mean,thick_metal_disp)*
           GFunction(tau-thick_tau_mean,thick_tau_disp)*
           double_exponential(R-R0,fabs(z)-z0,thick_Rd,thick_zd);
}
double new_prior_2018_broad_age::halo_prior(double R, double z, double Z, double tau){
    return halo_norm*
           GFunction(Z-halo_metal_mean,halo_metal_disp)*
           GFunction(tau-halo_tau_mean,halo_tau_disp)*
           pow(R*R+z*z+1e-2,-3.39/2.);
}
double new_prior_2018_broad_age::prior(VecDoub X, double Z, double tau){
    double R = sqrt(X[0]*X[0]+X[1]*X[1]), z = X[2];
    if(tau>13.) return 0.;
    return thin_prior(R,z,Z,tau)+thick_prior(R,z,Z,tau)+halo_prior(R,z,Z,tau)+bulge_prior(X,Z,tau);
}
//=============================================================================
double new_prior_2018_flat_age::bulge_prior(VecDoub X, double Z, double tau){
    double xrot = X[0]*cos(bulge_phi)-X[1]*sin(bulge_phi);
    double yrot = -X[0]*sin(bulge_phi)-X[1]*cos(bulge_phi);
    return bulge_norm*
           GFunction(Z-bulge_metal_mean,bulge_metal_disp)*
...
double new_prior_2018_flat_age::thin_prior(double R, double z, double Z, double tau){
    return thin_norm*
           GFunction(Z-thin_metal_mean,thin_metal_disp)*
           double_exponential(R-R0,fabs(z)-z0,thin_Rd,thin_zd);
}
double new_prior_2018_flat_age::thick_prior(double R, double z, double Z, double tau){
    return thick_norm*
           GFunction(Z-thick_metal_mean,thick_metal_disp)*
           double_exponential(R-R0,fabs(z)-z0,thick_Rd,thick_zd);
}
double new_prior_2018_flat_age::halo_prior(double R, double z, double Z, double tau){
    return halo_norm*
           GFunction(Z-halo_metal_mean,halo_metal_disp)*
           pow(R*R+z*z+1e-2,-3.39/2.);
}
double new_prior_2018_flat_age::prior(VecDoub X, double Z, double tau){
    double R = sqrt(X[0]*X[0]+X[1]*X[1]), z = X[2];
    if(tau>13.) return 0.;
    return thin_prior(R,z,Z,tau)+thick_prior(R,z,Z,tau)+halo_prior(R,z,Z,tau)+bulge_prior(X,Z,tau);
}
