...
public:
	double HW, FHALO, SIGFHALO, delta_halo_age=1.;
	...
	void setParams(VecDoub params){
		...
		if(params.size()>3){
			...
			if(params[7]!=0.) delta_halo_age=params[7];
		}
	}
	double chemDF_actions(const VecDoub& Actions, double F);
	double chemDF_actions(const VecDoub& Actions, double age, double F);
	double chemDF_real(const VecDoub& X, double F);
	double chemDF_real(const VecDoub& X, double age, double F);
