...
name_str = '_withPRIOR_FLAT_010219'
...
def run_apogee():
    apogee = load_apogee(output_file=output_folder + 'APOGEE_input%s.hdf5' % name_str, use_dr1=DR1)
...
def run_find_gridding(survey, output_name, unique_id):
    pp = pd.read_hdf(output_folder+'%s_input%s.hdf5'%(survey,name_str))
...
def combine_files(output_name='APOGEE'):
    ...
    pp.meta['COMMENT'] = pp.meta['COMMENT'] + ['Parallax zeropoint -0.05mas included',
					       'Flat age prior',
					       'systematic G error of 0.01 mag added'] 
    pp.write('/data/jls/GaiaDR2/spectro/APOGEE_distances_flatageprior_parzero05_sysG_forralph.hdf5',
